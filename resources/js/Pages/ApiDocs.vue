    ...
    <TacticalLayout :user="user">
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <div class="text-ui-sm font-mono text-upsilon-cyan/80 animate-pulse">
                    [ NODE_STATUS: SYNCED ]
                </div>
            </div>
        </template>
        ...
        <div class="py-6 px-4 max-w-7xl mx-auto">
            ...
            <div v-else class="space-y-16">
                ...
                <section v-if="workflow" class="border border-upsilon-cyan/20 bg-upsilon-gunmetal/20 p-6 shadow-neon">
                    <h3 class="font-scifi text-upsilon-cyan text-ui-md mb-4 tracking-tighter uppercase border-b border-upsilon-cyan/30 pb-2">
                        // OPERATIONAL_PROCEDURES
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div v-for="(steps, name) in workflow" :key="name" class="space-y-2">
                            <div class="text-ui-sm font-mono text-upsilon-magenta uppercase tracking-widest">{{ name }} process</div>
                            <div class="text-white/90 font-mono text-ui-sm leading-relaxed border-l-2 border-upsilon-magenta/50 pl-3">
                                {{ steps }}
                            </div>
                        </div>
                    </div>
                ...
                <section v-if="envelope" class="border border-upsilon-cyan/20 bg-upsilon-void p-6 relative">
                    <div class="absolute -top-3 left-6 bg-upsilon-void px-3 text-ui-sm font-mono text-upsilon-cyan uppercase border border-upsilon-cyan/30 shadow-glow-cyan">
                        Global_Result_Envelope
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                        <div class="space-y-3">
                            <div v-for="(desc, field) in envelope" :key="field" class="flex items-start gap-4">
                                <code class="text-upsilon-cyan font-bold font-mono text-ui-sm min-w-[100px]">{{ field }}</code>
                                <span class="text-white/80 text-ui-sm font-sans leading-tight">{{ desc }}</span>
                            </div>
                        </div>
                        <div class="bg-black/40 p-4 border border-upsilon-cyan/10 rounded">
                            <pre class="text-ui-sm font-mono text-upsilon-lime leading-tight">
{
  "request_id": "018f...",
                ...
                <div>
                    ...
                    <div class="mb-12 relative group max-w-3xl mx-auto">
                        ...
                        <div class="absolute inset-y-0 right-6 flex items-center pointer-events-none opacity-60">
                            <span class="text-ui-sm font-mono text-white">[ SEARCH_QUERY ]</span>
                        </div>
                            ...
                            <div class="grid grid-cols-1 gap-8">
                                ...
                                >
                                    ...
                                    <div class="flex flex-wrap items-center gap-6 mb-4">
                                        <span :class="['font-mono font-bold px-4 py-1 border text-ui-md uppercase tracking-widest', getVerbClass(endpoint.methods)]">
                                        ...
                                        <div v-if="endpoint.auth?.required" class="flex items-center gap-2 text-ui-sm font-mono text-upsilon-magenta bg-upsilon-magenta/20 px-2 py-1 border border-upsilon-magenta/40 shadow-glow-magenta">
                                            <span class="animate-pulse">●</span> AUTH_REQUIRED
                                            <span v-if="endpoint.auth?.admin_only" class="border-l border-upsilon-magenta/30 pl-2 uppercase font-bold text-ui-xs">ROOT_ONLY</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                                        ...
                                        <div v-if="endpoint.parameters && endpoint.parameters.length > 0">
                                            <div class="text-ui-sm font-mono text-upsilon-cyan/80 uppercase mb-3 tracking-[0.2em] font-bold">Input_Contracts</div>
                                            <div class="grid grid-cols-1 gap-2">
                                                <div v-for="param in endpoint.parameters" :key="param.name" class="flex flex-col p-3 bg-upsilon-void/80 border border-upsilon-cyan/20">
                                                    <div class="flex items-center justify-between mb-1">
                                                        <code class="text-upsilon-magenta font-bold font-mono text-ui-sm">{{ param.name }}</code>
                                                        <span class="text-ui-xs font-mono text-upsilon-cyan bg-upsilon-cyan/10 px-1 uppercase">{{ param.type }}</span>
                                                    </div>
                                                    <div class="text-white/80 text-ui-xs font-mono truncate">{{ param.constraints }}</div>
                                                    <div v-if="param.required" class="mt-1 text-ui-xs font-mono text-upsilon-magenta uppercase font-bold">Required</div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div v-if="endpoint.output">
                                            <div class="text-ui-sm font-mono text-upsilon-cyan/80 uppercase mb-3 tracking-[0.2em] font-bold">Output_Payload</div>
                                            <div v-if="getDto(endpoint.output)" class="bg-upsilon-void/80 border border-upsilon-lime/40 p-3 shadow-glow-lime">
                                                <div class="text-ui-sm font-mono text-upsilon-lime mb-2 border-b border-upsilon-lime/20 pb-1 flex justify-between">
                                                    <span>DTO: {{ endpoint.output }}</span>
                                                    <span class="animate-pulse text-ui-sm">✓</span>
                                                </div>
                                                <div class="space-y-1">
                                                    <div v-for="prop in getDto(endpoint.output).properties" :key="prop.name" class="flex items-center justify-between">
                                                        <code class="text-white font-mono text-ui-sm">{{ prop.name }}</code>
                                                        <span class="text-upsilon-lime font-mono text-ui-xs uppercase">{{ prop.type }}</span>
                                                    </div>
                                                    <div v-if="getDto(endpoint.output).properties.length === 0" class="text-ui-sm font-mono text-white/60 italic">
                                                        [ COMPLEX_JSON_PAYLOAD ]
                                                    </div>
                                                </div>
                                            </div>
                                            <div v-else class="text-ui-sm font-mono text-upsilon-magenta italic bg-upsilon-magenta/5 p-3 border border-upsilon-magenta/20">
                                    ...
                                    <div v-if="endpoint.atd_link" class="absolute top-6 right-6 opacity-40 group-hover:opacity-100 transition-opacity">
                                        <div class="text-ui-xs font-mono text-white flex flex-col items-end">
                ...
                <section v-if="websockets" class="space-y-12 pb-20">
                    ...
                    <div class="bg-upsilon-void/80 border border-upsilon-cyan/40 p-8 relative overflow-hidden shadow-neon">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                            <!-- Connection Details -->
                            <div>
                                <h4 class="text-ui-sm font-mono text-upsilon-magenta mb-6 uppercase tracking-widest font-bold">Connection_Protocol</h4>
                                <div class="space-y-4">
                                    <div class="p-3 bg-upsilon-gunmetal/30 border-l-2 border-upsilon-magenta/60">
                                        <div class="text-ui-sm font-mono text-upsilon-magenta/80 mb-1">HANDSHAKE</div>
                                        <div class="text-ui-sm text-white leading-tight italic">{{ websockets.handshake }}</div>
                                    </div>
                                    <div class="grid grid-cols-2 gap-2 text-ui-sm font-mono">
                            ...
                            <div class="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div v-for="channel in websockets.channels" :key="channel.pattern" class="space-y-6">
                                    <div>
                                        <code class="text-upsilon-cyan font-bold font-mono text-ui-md block mb-1">/ {{ channel.pattern }}</code>
                                        <p class="text-ui-sm text-white/90 leading-relaxed font-sans">{{ channel.description }}</p>
                                    </div>
                                    ...
                                    <div class="space-y-4">
                                        <div v-for="event in channel.events" :key="event.name" class="p-4 bg-upsilon-gunmetal/20 border border-upsilon-cyan/20 group/event hover:border-upsilon-magenta/50 transition-all">
                                            <div class="text-ui-sm font-mono text-upsilon-magenta mb-1 font-bold tracking-widest">{{ event.name }}</div>
                                            <p class="text-ui-sm text-white/80 mb-3">{{ event.description }}</p>
                                            <div class="text-ui-xs font-mono">
                                    ...
                                    <div class="text-ui-xs font-mono text-upsilon-cyan animate-pulse italic">
