...
parents:
  - [[upsilonapi:api_go_battle_engine]]
dependents:
  - [[entity_api_dtos]]
---
...
- **Envelope Adherence:** All communications must be wrapped/unwrapped using the [[upsilonapi:api_standard_envelope]].
- **DTO Mapping:** The service must use [[entity_api_dtos]] for all request payloads and response unpacking.
...
- **Spec Links:**
    - [[upsilonapi:api_go_battle_start]] -> `startArena(ArenaStartRequest $dto)`
    - [[upsilonapi:api_go_battle_action]] -> `sendAction(ArenaActionRequest $dto)`
- **Code Tag:** `@spec-link [[module_upsilon_api_service]]`
